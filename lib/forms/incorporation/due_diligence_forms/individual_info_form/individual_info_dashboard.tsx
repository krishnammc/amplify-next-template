"use client"
import { FLEX_BORDER_COLOR, HEADING_ICON_ACTIVE_COLOR, HELP_TEXT_COLOR, TEXT_COLOR, BUTTON_LINEAR_LEFT_COLOR, BUTTON_LINEAR_RIGHT_COLOR, BUTTON_TEXT_COLOR, FILE_BG, FILE_BORDER_COLOR, HEADER_ICON_COLOR, PAGE_HEADING_TEXT_COLOR, LABEL_TEXT_FONT_WEIGHT, TEXT_FONT_SIZE, LIST_TEXT_FONT_SIZE, TEXT_FONT_WEIGHT, BUTTON_TEXT_FONT_WEIGHT, POPUP_HEADER_FONT_SIZE, BUTTON_BG, FLEX_BUTTON_BG, POPUP_FORM_BG, SUB_HEADING_FONT_SIZE, FORM_MAX_WIDTH, POPUP_FORM_MAX_WIDTH, LIST_FORM_DATA_MAX_WIDTH, FORM_BORDER_COLOR } from '@/lib/app/app_constants';
import { Flex, Button, Text, Modal, ModalBody, ModalContent, ModalOverlay, useDisclosure, Accordion, AccordionButton, AccordionIcon, AccordionItem, AccordionPanel } from '@chakra-ui/react';
import React, { useState } from 'react'
import { FaInbox } from 'react-icons/fa6';
import { IoCloseCircleOutline } from 'react-icons/io5';
import AddIndividual from './add_individual';
...
import { IndividualInfoListData } from '@/lib/api/incorporation/due_diligence_form_data/individual_info_data';
...
const IndividualInfoDashboard = ({ answer, page_values }: fields) => {
  const { isOpen, onOpen, onClose } = useDisclosure();
  const [listEmpty, SetListEmpty] = useState<boolean>(false);
  const IndividualInfoList = IndividualInfoListData;

  const section_heading = ["Add Individual", "Political Exposed Person (PEP) Info", "Enhanced CDD Info"];
    ...
    <Flex w = {'100%'} >
      { listEmpty ?
        <Flex flexDir = {'column'} maxW = {FORM_MAX_WIDTH} w={'100%'} h = {'335px'} p = {'20px'} gap = {['10px', '24px']} borderRadius = {'4px'} borderColor = {FORM_BORDER_COLOR}>
          <Text fontWeight = {LABEL_TEXT_FONT_WEIGHT} fontSize = {TEXT_FONT_SIZE}>List of Individual</Text>
          <Flex w = {'100%'} flexDir = {'column'} p = {['20px', '20px', '30px']} gap = {'20px'} justifyContent = {'center'} h = {'100%'} alignItems = {'center'}>
            ...
            <Flex flexDir = {'column'} gap = {'5px'} justifyContent = {'flex-end'} >
              <Text textAlign = {'center'} fontSize = {TEXT_FONT_SIZE} fontWeight = {LABEL_TEXT_FONT_WEIGHT} color = {HELP_TEXT_COLOR}>There are no Individual data yet</Text>
              <Text textAlign = {'center'} fontSize = {LIST_TEXT_FONT_SIZE} fontWeight = {TEXT_FONT_WEIGHT} color = {HELP_TEXT_COLOR}>Click on the Add New Individual button to fill the data</Text>
            </Flex>
          </Flex>
          ...
          <Flex justifyContent = {'center'} alignItems = {'flex-end'}>
            <Button
              ...
              onClick = {onOpen}
            >
              <Text color = {BUTTON_TEXT_COLOR} fontSize = {TEXT_FONT_SIZE} fontWeight = {BUTTON_TEXT_FONT_WEIGHT}>Add New Individual</Text>
            </Button>
          </Flex>
        </Flex>
        :
        <Flex w = {'100%'} flexDir = {'column'}   p = {'20px'} gap = {'24px'}   borderWidth = {'1px'} borderRadius = {'4px'} borderColor = {FLEX_BORDER_COLOR}>
          <Flex justifyContent = {'space-between'} alignItems = {'center'}>
            ...
            <Button 
              ...
              onClick = {onOpen}
            >
              <Text color = {BUTTON_TEXT_COLOR} display = {["none","none","flex","flex","flex"]} fontSize = {TEXT_FONT_SIZE}  fontWeight = {BUTTON_TEXT_FONT_WEIGHT}>Add New Individual</Text>
                    ...
                    <Flex w = {'100%'} flexDir = {'column'} gap = {'12px'} >
                      ...
                      <Flex w= {'100%'} justifyContent = {'space-between'} gap={'15px'}>
                        <Flex w = {'50%'}  flexDir = {'column'} fontSize = {TEXT_FONT_SIZE} gap = {'4px'} textAlign = {'left'}>
                          <Text fontWeight = {LABEL_TEXT_FONT_WEIGHT} color = {BUTTON_TEXT_COLOR}>Acting Capacity</Text>
                          <Text fontWeight = {TEXT_FONT_WEIGHT}>{field.acting_capacity}</Text>
                        </Flex>
                      ...
                      <Flex w= {'100%'} justifyContent = {'space-between'} gap={'15px'} >
                        <Flex w = {'50%'} flexDir = {'column'} fontSize = {TEXT_FONT_SIZE} gap = {'4px'} textAlign = {'left'}>
                          <Text fontWeight = {LABEL_TEXT_FONT_WEIGHT} color = {BUTTON_TEXT_COLOR}>Gender</Text>
                          <Text fontWeight = {TEXT_FONT_WEIGHT}>{field.gender}</Text>
                        </Flex>
                        <Flex w = {'50%'}  flexDir = {'column'} fontSize = {TEXT_FONT_SIZE} gap = {'4px'} textAlign = {'right'}>
                      ...
                      <Flex w= {'100%'} justifyContent = {'space-between'} gap={'15px'} >
                        <Flex w = {'100%'} flexDir = {'column'} fontSize = {TEXT_FONT_SIZE} gap = {'4px'} textAlign = {'left'}>
                          <Text fontWeight = {LABEL_TEXT_FONT_WEIGHT} color = {BUTTON_TEXT_COLOR}>Intended Nature and Purpose of Business Relationship</Text>
                          <Text fontWeight = {TEXT_FONT_WEIGHT}>{field.intended_nature_and_purpose_of_business_relationship}</Text>
                        </Flex>
                      </Flex>
                  ...
                  if (section.section_heading == section_heading[0]) {
                    return (
                      <>
                        ...
                        <AddIndividual section_values = {section.section_values} answer_data = {answer} />
                      </>
                    );
                  }
...
export default IndividualInfoDashboard
