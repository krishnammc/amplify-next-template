...
import { BOTTOM_BUTTON_BORDER_COLOR, BUTTON_BG, FILE_BORDER_COLOR, HELP_TEXT_COLOR, TEXT_FONT_SIZE, BUTTON_TEXT_FONT_WEIGHT, BUTTON_LINEAR_LEFT_COLOR, BUTTON_LINEAR_RIGHT_COLOR, BUTTON_TEXT_COLOR, TOP_BUTTON_TEXT_COLOR } from '@/lib/app/app_constants';
import { requirementFormPathToNextPath, requirementFormPathToPrevPath, requirementFormsPath, requirementFormsPrevPathName } from '@/lib/app/constant_data';

const RequirementFormBottomButton = () => {
  const router = useRouter();

  //Getting Current Path
  const currentPath = usePathname() as requirementFormsPath;

  //Current Path to Path Heading
  const pathNameToText = requirementFormsPrevPathName;
  const buttonText = pathNameToText[currentPath];

  //Current Path to PrevPath Data
  const pathToPrevPath = requirementFormPathToPrevPath;
  const prevPath = pathToPrevPath[currentPath];

  //Current Path to NextPath Data
  const pathToNextPath = requirementFormPathToNextPath;
    ...
    <Flex position = {'fixed'} bottom = {'0'} w  = {'100vw'} h = {'80px'} justifyContent = {'space-between'} p = {'20px 40px 20px 40px'} borderTop = {'1px'} borderColor = {BOTTOM_BUTTON_BORDER_COLOR} bg = {BUTTON_BG}>
      {
        ...
        :
          <Button w = {'auto'} h = {'40px'} gap = {'5px'} borderColor = {FILE_BORDER_COLOR} borderWidth = {'1px'} borderRadius = {'6px'} bg = {BUTTON_BG} onClick = {MoveToPrevPath}>
            <Text color = {TOP_BUTTON_TEXT_COLOR} fontSize = {TEXT_FONT_SIZE} fontWeight = {BUTTON_TEXT_FONT_WEIGHT}>Back - {buttonText}</Text>
          </Button>
      }
...
export default RequirementFormBottomButton
