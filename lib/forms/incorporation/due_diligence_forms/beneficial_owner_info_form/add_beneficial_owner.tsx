...
import { DueDiligenceFormSectionValues, InputFieldType } from '@/lib/interfaces/incorporation/due_diligence_form/interfaces';
...
const AddBeneficialOwner = ({section_values, answer_data}:fields) => {
              ...
              section_values.map((e:DueDiligenceFormSectionValues) => {
                ...
                switch (e.type) {
                  ...
                  case "TEXTAREA" :
                    return(
                      <GridItem colSpan = {[2,1]}>
                          <TextAreaField label = {e.label} value = {stateValue}  values = {e.values} helpText = {e.help_text} format = {e.format_validation} inputProps = {{ onChange: event => onChangeTextArea(event, e.id, e) }} isInValid = {isInValid} required = {e.is_required} toolTip = {e.label_tooltip} errorMessage = {errorMessage} h = {'78px'} />
                          </GridItem>
                    );
                    case "FULL_TEXTAREA" :
                    return(
