import { shareHoldingTableAnswerData } from "@/lib/api/incorporation/requirement_form_data/share_holder_data";
import { TEXT_FONT_SIZE, LABEL_TEXT_FONT_WEIGHT, TEXT_COLOR, LABEL_COLOR, LABEL_TEXT_FONT_SIZE, TEXT_FONT_WEIGHT, BUTTON_TEXT_FONT_WEIGHT, FILE_BORDER_COLOR, HEADER_ICON_COLOR } from "@/lib/app/app_constants";
...
import { DynamicInputFieldType, FieldValidationType, RequirementFormSectionValues, RequirementFormTableValues } from "@/lib/interfaces/incorporation/requirement_form/interfaces";
import { getNumberFromString, validateField } from "@/lib/utlils/utill_methods";
import { Button, Flex, FormLabel, GridItem, SimpleGrid, Text } from "@chakra-ui/react";
import React from "react";
import { useState, ChangeEvent } from "react";

export interface fields {
  section_values:RequirementFormSectionValues[],
  answer_data:Record<string, string | number | string[] | number[] | boolean>
}

const AddNewDetails = ({section_values,answer_data}:fields) => {

  const tableAnswerData = shareHoldingTableAnswerData;

  const answerDataArray = (label:Record<string, string | number | string[] | number[] | boolean>) => {
  ...
  return (
    <Flex flexDir = {'column'} >
      <Text fontSize = {TEXT_FONT_SIZE} fontWeight = {LABEL_TEXT_FONT_WEIGHT} color = {TEXT_COLOR}>{/* {heading} */}</Text>
          ...
          <SimpleGrid columns = {2} columnGap = {'12px'} rowGap = {'12px'}>
            {
              section_values.map((e:RequirementFormSectionValues) => {  
                ...
                switch(e.type) {
                  ...
                  case "STATIC_TEXT_WITH_BUTTON" :
                    return(
                      <GridItem key = {e.id} colSpan = {2}>
                        <Flex mt = {'15px'} flexDir = {'row'} h = '32px' justifyContent = {'space-between'} alignItems = {'center'}>                      
                          <FormLabel  fontSize = {LABEL_TEXT_FONT_SIZE} fontWeight = {LABEL_TEXT_FONT_WEIGHT} color = {LABEL_COLOR} >List of Shareholders</FormLabel>
                          <Button  borderColor = {FILE_BORDER_COLOR} h = {'32px'} borderWidth = {'1px'} bg = {HEADER_ICON_COLOR}>
                            <Text fontSize={TEXT_FONT_SIZE} fontWeight={BUTTON_TEXT_FONT_WEIGHT}>Add Existing Shareholder</Text>
                          </Button>
                        </Flex>
                      </GridItem>
                    );
                  ...
                  case "TABLE" :
                    return (
                      <GridItem key = {e.id} colSpan = {2}>
                        <TableField label = {e.label} value = {stateValue} values = {e.values} tableValues = {e.table_values} answerData = {tableAnswerData} helpText = {e.help_text} format = {e.format_validation} inputProps = {{ onChange: event => onChange(event, e.id, e) }} isInValid = {isInValid} required = {e.is_required} toolTip = {e.label_tooltip} errorMessage = {errorMessage} isShareHolder = {true} />
                      </GridItem>
                    );
                  case "SUB_TABLE" :
                    return (
                      <GridItem key = {e.id} colSpan = {2}>
                        <SubTableField label = {e.label} value = {stateValue} values = {e.values} helpText = {e.help_text} format = {e.format_validation} inputProps = {{ onChange: event => onChange(event, e.id, e) }} isInValid = {isInValid} required = {e.is_required} toolTip = {e.label_tooltip} errorMessage = {errorMessage} isShareHolder = {true}/>
                      </GridItem>
                    );
                }              
              })
