...
import { FORM_BORDER_COLOR } from "@/lib/app/app_constants";
...
import ProposeCompany from "@/lib/components/propose_company";
...
const CompanyInfo  = ({section_values,answer_data}:fields) => {
  ...
  /* const isHidden = (data: string, answers:Answer[]) => {
    const ids = data.split('-');
    const isContainsValueId = ids.length > 1;

    const answer = answers.find((e: { id: { toString: () => string; }}) => e.id.toString() == ids[0].toString());
    if (typeof answer?.answer == 'number') return false;

    if (isContainsValueId) {
      const valueId = ids[1];
      if (typeof answer?.answer == 'string') return answer == null || answer.answer.length < 1 || answer.answer != valueId;
      if (typeof answer?.answer != 'string' && typeof answer?.answer != 'number' || Array.isArray(answer?.answer)) {
        const answerArray = Array.isArray(answer?.answer) ? answer?.answer.map(e => e.toString()) : [];
        const valueIds = ids.slice(1).map(e => e.toString())
        return answer == null || answer.answer.length < 1 || valueIds.some(valueId => answerArray.includes(valueId)) == false
      }
    } 
    return answer == null || answer?.answer.length < 1;
  } */
    ...
    tempData.forEach((input) => {
      ...
      value.map((e) => { 
        if (e.type == "TEXT") {
          ...
          const validateResult = validateField(input.value as string , e.format_validation)
          input.error = e.is_required ? (validateResult.isEmpty? "EMPTY" : validateResult.isContainsFormatError? "FORMAT" : null) : null;
        } else if (e.type == "PROPOSE_COMPANY_TEXT") {
          let value: number | string = input.value as number | string;
          const isNumberRelatedInput = e.format_validation !== null ? ['NUMBER', 'NUMBER_WITH_LIMIT', 'PRICE', 'PRICE_WITH_LIMIT'].includes(e.format_validation) : false;
          const validateResult = validateField(value.toString(), e.format_validation, isNumberRelatedInput ? e.limit.toString() ?? '' : undefined)
          input.error = e.is_required ? (validateResult.isEmpty? "EMPTY" : validateResult.isContainsFormatError? "FORMAT" : null) : null;
        } else if (e.type == "PROPOSE_COMPANY_SELECT") {
          const validateResult = validateField(input.value as string, e.format_validation)
          input.error = e.is_required ? (validateResult.isEmpty? "EMPTY" : validateResult.isContainsFormatError? "FORMAT" : null) : null;
        } else if (e.type == "FINANCIAL_SELECT") {
          const validateResult = validateField(input.value as string, e.format_validation)
          input.error = e.is_required ? (validateResult.isEmpty? "EMPTY" : validateResult.isContainsFormatError? "FORMAT" : null) : null;
        } else if (e.type == "FINANCIAL_DATE") {
          const validateResult = validateField(input.value as string, e.format_validation)
          input.error = e.is_required ? (validateResult.isEmpty? "EMPTY" : validateResult.isContainsFormatError? "FORMAT" : null) : null;
    ...
    <Flex w = {'100%'} flexDir = {'column'} >
      <form onSubmit = {onSubmit}>
        <Flex w = {'100%'} flexDir = {'column'} gap = {'12px'}>
          <SimpleGrid w = {'100%'} columns = {6} columnGap = {'12px'} rowGap = {'12px'}>
            {
              section_values.map((e:RequirementFormSectionValues) => {  
                ...
                switch(e.type) {
                  ...
                  case "PROPOSE_COMPANY_TEXT" :
                    return(
                      <GridItem key = {e.id} colSpan = {[6,4]}>
                        <TextField label = {e.label} value = {stateValue}  values = {e.values} helpText = {e.help_text} format = {e.format_validation} inputProps = {{ onChange: event => onChange(event, e.id, e) }} isInValid = {isInValid} required = {e.is_required} toolTip = {e.label_tooltip} errorMessage = {errorMessage} h = {'44px'} />        
                      </GridItem>
                    );
                  case "PROPOSE_COMPANY_SELECT" :
                    return(
                  ...
                  case "LABEL_TEXT" :
                    return(
                      <GridItem key = {e.id} colSpan = {6} columnGap = {'8px'} borderWidth = {'1px'} borderColor = {FORM_BORDER_COLOR} borderRadius = {'6px'} >
                        <ProposeCompany/>  
                      </GridItem>
                    );
                  case "FINANCIAL_DATE" :
