...
import { PiNumberCircleFiveFill, PiNumberCircleFourFill, PiNumberCircleOneFill, PiNumberCircleThreeFill, PiNumberCircleTwoFill } from 'react-icons/pi';
...
import { HEADER_BG, FORM_MAX_WIDTH, HEADING_ICON_ACTIVE_COLOR, HEADER_ICON_COLOR, HEADING_ICON_SIZE, TEXT_FONT_SIZE, TEXT_FONT_WEIGHT, BUTTON_BG } from '@/lib/app/app_constants';
import { requirementFormPathName, requirementFormPathTitle } from '@/lib/app/constant_data';
...
const RequirementFormHeader = () => {
  ...
  //Path Information
  const pathTitle = requirementFormPathTitle;
  const pathName = requirementFormPathName;

  useEffect(() => {
    const currentIndex = pathName.findIndex(route => `/requirement_form/${route}` === currentPath);
  ...
  return (
    <>
      ...
      <Flex display = {['none','none','none','flex']} flexDir = {'column'} w = {'100vw'} bg = {HEADER_BG} h = {'64px'} color = {'white'} >
        ...
        <Flex p = {'20px 40px 20px 40px'} justifyContent = {'space-between'} alignItems = {'center'}>
          {/*  Flash Logo */}
          ...
          {/* Form Headings */}
          <Flex maxW = {FORM_MAX_WIDTH} w = {'100%'} h = {'24px'} gap = {'8px'}>
            {
              pathTitle.map((path, index) => {
                const isActive = `/requirement_form/${pathName[index]}` === currentPath;
                return (
